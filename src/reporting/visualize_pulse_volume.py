...
def visualize_pulse_volume():
    data_dir = "data/synthetic_transcripts/velofit/full_scale_run"
    if len(sys.argv) > 1:
        data_dir = sys.argv[1]

    print(f"Scanning {data_dir} for Pulse Item Calls...")
    ...
    # Sort and Fill Dates
    if not daily_counts:
        print("No Pulse calls found in range.")
        return

    sorted_days = sorted(daily_counts.keys())
    
    print("\n--- Pulse Item Call Volume (Dec 1 - Jan 28) ---")
