...
def upload_single_file(blob, local_path):
    ...
    try:
        ...
        for filename in os.listdir(output_dir):
            if filename.endswith(".json"):
                ...
                files_to_upload.append((blob, local_path))
        
        print(f"Found {len(files_to_upload)} files to upload. Starting parallel upload with 10 workers...")
        
        # Parallel execution
        with ThreadPoolExecutor(max_workers=10) as executor:
            # We use map to ensure exceptions are raised if they occur
            # But since upload_single_file has retry logic, it should handle transient errors
            futures = [executor.submit(upload_single_file, blob, local_path) for blob, local_path in files_to_upload]
            ...
            for future in futures:
                try:
                    ...
                    print(f"One of the uploads failed: {e}")
                    # We might want to raise here or continue, depending on desired behavior
                    # For now, let's log and re-raise to fail the script if any file fails
                    raise e
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_prefix", type=str, required=True, help="GCS prefix")
    ...
    # Update GCS prefix
    dataset_prefix = args.dataset_prefix if args.dataset_prefix.endswith('/') else f"{args.dataset_prefix}/"
    ...
    # 2. Upload Results
    try:
        upload_json_files(args.bucket_name, dataset_prefix, args.output_dir)
    except Exception as e:
